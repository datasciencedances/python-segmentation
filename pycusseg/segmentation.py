...
import matplotlib.gridspec as gridspec
def get_score_kmean(data, start_k, stop_k):
    silhouette_scores = []
    inertias = []
...
def get_score_kmodes(data, start_k, stop_k):
...
def plot_elbow_method(values, start_k, stop_k, title="Elbow Method", ylabel="Inertia (Tổng phương sai nội cụm)"):
...
def plot_silhouette_score(silhouette_scores, start_k, stop_k):
    sns.set(style='darkgrid', rc={'axes.facecolor': '#fcf0dc'})
    sns.set_palette(['darkorange'])
    fig = plt.figure(figsize=(20, 5))
